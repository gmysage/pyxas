...
def train_1_branch_bkg(model_gen, dataloader, loss_r, device='cuda:1', lr=1e-4, train_fit=False, take_log=True):
    ...
    for bi, data in tqdm(enumerate(dataloader), total=int(len(dataloader.dataset)/batch_size)):
        ...
        loss_value['vgg_identity'] = vgg_loss(output_bkg, label, device=device)
        
        # r_vgg_fit:
        loss_value['vgg_fit'] = vgg_loss(y_fit_reshape, real_img, device=device)
        ...
        loss_value['vgg_1st_last'] += vgg_loss(im1, im2, device=device)
        '''
