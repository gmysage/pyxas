...
def ml_xanes3D(sino_sum, r_shuffle, loss_r, n_epoch, angle_l, lr=0.1):
    ...
    for epoch in trange(n_epoch):
        ...
        #loss_val['tv_sino'] = pyxas.tv_loss_norm(sino_dif[:,:,:, :sino_col]) + pyxas.tv_loss_norm(sino_dif[:,:,:, -sino_col:])
        loss_val['tv_sino'] = pyxas.tv_loss_norm(sino_dif)
        loss_val['tv_img'] = pyxas.tv_loss_norm(guess1) + pyxas.tv_loss_norm(guess2)
...
def ml_tomo_general(guess, sino_sum_general, loss_r, n_epoch, angle_l, lr=0.1, threshold=0):
    ...
    for epoch in trange(n_epoch):
        ...
        loss_val['tv_sino'] = pyxas.tv_loss_norm(sino_dif)
        loss_val['tv_img'] = pyxas.tv_loss_norm(guess)
...
def ml_xanes3D_general(guess, sino_sum_general, r_shuffle_general, loss_r, n_epoch, angle_l, lr=0.1, row_range=None):
    ...
    for epoch in trange(n_epoch):
        ...
        if not row_range is None:
            loss_val['tv_sino'] = pyxas.tv_loss_norm(sino_dif[row_range[0]: row_range[1]])
        else:
            loss_val['tv_sino'] = pyxas.tv_loss_norm(sino_dif)
        loss_val['tv_img'] = pyxas.tv_loss_norm(guess)
        loss_val['l1_sino'] = pyxas.l1_loss(sino_sum_general, sino_out)
...
def test_phantom():
    ...
    for epoch in trange(n_epoch):
        ...
        loss_tv = pyxas.tv_loss_norm(sino_out)
    ...
    for epoch in trange(n_epoch):
        ...
        loss_tv = (pyxas.tv_loss_norm(sino_out1 + sino_out2 - sino_sum)) / 128 / n_angle
